    ...
    public static Ascensor newAscensor(){
        try {
            ...
            throw new RuntimeException(
                "No se puede crear una nueva instancia de la clase: ",e);
        }
    }
        ...
        for(Method metodo: clase.getDeclaredMethods()){
            if(metodo.getAnnotations()!= null){
                ...
                if(metodo.getAnnotation(PostConstruct.class) != null){
                    PostConstruct anotacion= metodo.getAnnotation(
                            PostConstruct.class);
                    
                    // se llama un determindado numero de veces el metodo
                    for (int i = 0; i < anotacion.call()-1; i++) {
                        sb.append("super."+ metodo.getName()+ "(");
                         for(Parameter parametro: metodo.getParameters()){
                             sb.append(parametro.getName());
                         }
                ...
                if(metodo.getAnnotation(Log.class) != null){
                    Log anotacion= metodo.getAnnotation(
                            Log.class);
